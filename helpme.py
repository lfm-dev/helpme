...
def color_to_comments(guide_text) -> None:
    ...
    for line_index, line in enumerate(guide_text):
        ...
        if not line.startswith('#') and '# ' in line:
            guide_text[line_index] = f'{line[:line.find(" #")]} `{line[line.find(" #")+1:]}`' # so it is recognized as a python inline code block
    guide_text = ('\n').join(guide_text)
    return guide_text

def print_chosen_guide_content(fullpath: str) -> None:
    '''
...
def get_hits(queries: list) -> list:
    '''
